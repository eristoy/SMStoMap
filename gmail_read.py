...
for mssg in mssg_list:
    ...
    try:
        ...
        #print (msg_from,mssg_body)
        #match = re.search('https://(.*) ',mssg_body)
        #res = match.group(1)
        r = re.compile('from (.*)YOUR')
        res = filter(r.match,mssg_body)
        s = ''.join(mssg_body)
        ...
        pass
    #print ("This is the temp dict")
    #print (temp_dict)
    Date, Snip, Sndr, Sbj = [temp_dict.get(k) for k in ['Date', 'Snippet', 'Sender', 'Subject']]
    #print "Date is %s" % (Date)
    #print (Snip)
    match = re.search('gl=[a-z]{2}(.*) YOUR', Snip)
    if match:
        TXT = match.group(1)
    else:
    ...
    LON = geo.group(1)
    LAT = geo.group(2)
    print '%s, %s, %s, %s, %s' % (Date, Sndr, LON, LAT, TXT)
